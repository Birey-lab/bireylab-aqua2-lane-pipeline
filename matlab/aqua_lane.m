...
function aqua_lane(pIn, pOut, cfgFile)
% ===== COMPILED LANE RUNNER =====
% Standalone: runs on the free MATLAB Runtime with NO license checkout (no Emory login).
% Usage (compiled):  aqua_lane "C:\path\lane01" "C:\path\lane01_results" ["C:\AQuA2\cfg\parameters_for_batch.csv"]
% Paths may be passed without a trailing slash; the function normalizes pOut.
if nargin < 2 || isempty(pIn) || isempty(pOut)
    error('Usage: aqua_lane <pIn> <pOut> [cfgFile]');
end
if nargin < 3 || isempty(cfgFile)
    cfgFile = 'C:\AQuA2\cfg\parameters_for_batch.csv';   % external CSV: edit it without recompiling
end
...
batchSet.outputRisingMap = false;   % rising maps OFF: software-rendered (no GPU) and slow; not needed here
...
for xxx = 1:numel(files)
    f1 = files(xxx).name; 
    % --- resume guard: skip files already completed (cheap restart after interruption) ---
    name_chk = erase(f1, {'.tiff','.tif','.mat'});
    donePath = fullfile(pOut, [name_chk '_results'], [name_chk '_AQuA2.mat']);
    if exist(donePath, 'file')
        fprintf('[skip] %s already done\n', f1);
        continue;
    end
    try   % per-file guard: a bad file is logged and skipped, lane continues
    %% load setting (you can also manually modify setting here)
    opts = util.parseParam_for_batch(1, [], cfgFile);  % File1 column; CSV read from disk at runtime
...
%     opts.detectGlo = true;
    ...
    if batchSet.outputFeatureTable
        ...
        %% rising maps  (controlled by batchSet.outputRisingMap; default OFF)
    ...
    if batchSet.outputMovie
        ...
        try
            ...
            fid = fopen(fullfile(failDir, [erase(f1,{'.tiff','.tif','.mat'}) '_ERROR.txt']), 'w');
            fprintf(fid, 'FILE: %s\n\n%s\n', f1, getReport(ME,'extended'));
            fclose(fid);
        catch; end
        continue;
    end
