# 09 — AMI Quick Launch (v0.8.3 fast path)

This is the **recommended entry point** for new users with access to the pre-built lab AMI `Windows2025-AQuA2-Pipeline-v3` (`ami-03473aa6f1cc13fbc`, us-east-1). It bypasses the manual infrastructure setup ([`02_INFRASTRUCTURE_SETUP.md`](02_INFRASTRUCTURE_SETUP.md)) and most of the multi-script operations ceremony ([`04_PIPELINE_OPERATIONS.md`](04_PIPELINE_OPERATIONS.md) — retained for advanced/debugging use) by leveraging the `Run-Pipeline.ps1` orchestrator (v0.8.3+).
