# ============================================================================
# AQuA2 CFU ANALYSIS PIPELINE (hCO / assembloid) -- v4.26
...
# NULL = keep all values for that field.  Examples:
#   INCLUDE_CONDITIONS <- c("C")           # Control only
#   INCLUDE_DONORS     <- c("1")           # Donor 1 only
#   INCLUDE_TIMEPOINTS <- c("D80","D120")  # primary timepoints only
INCLUDE_DONORS      <- c("1", "4")               # NULL or character vector
INCLUDE_CONDITIONS  <- c("C", "G", "L")          # C=Control, G=GoF, L=LoF
INCLUDE_TIMEPOINTS  <- c("D80","D100","D120")    # NULL to keep all
INCLUDE_PROMOTERS   <- NULL                       # [v4.26] NULL = keep all promoters; e.g. c("CAMK2A") or c("DLX")
...
write_prism_exports <- function(per_cell_csv_path, out_root, df = NULL) {
  ...
  df <- as.data.frame(df)
  if (!all(c("Condition", "Donor") %in% names(df))) {
    message("[v4.23] PRISM export skipped: per-cell summary lacks Donor/Condition columns.")
    return(invisible(NULL))
  }
  # Map condition codes to PRISM-friendly labels
  cond_map <- c("C" = "Control", "G" = "GoF", "L" = "LoF")
  df$.CondLabel <- cond_map[as.character(df$Condition)]
  # Identify metric columns: numeric & not metadata / ID-style
  meta <- c("File","Donor","Condition","DonorCond",".CondLabel","AgeDay","Timepoint","CondTimepoint",
            "Mag","NominalHz","EffectiveHz","Tissue","Promoter","Organoid","Video","CFU_ID",
            paste0("ID", 1:20))
  ...
  cohorts <- list(
    Donor1   = df[!is.na(df$Donor) & df$Donor == "1", , drop = FALSE],
    Donor4   = df[!is.na(df$Donor) & df$Donor == "4", , drop = FALSE],
    Combined = df
  )
  # [v4.26] Optional per-promoter cohorts: Donor1_CAMK2A, Donor1_DLX,
  # Donor4_CAMK2A, Donor4_DLX, Combined_CAMK2A, Combined_DLX (or whichever
  # promoter values are actually present in the data).  Sibling folders of
  # the promoter-mixed cohorts above.
  if (isTRUE(get0("PRISM_BY_PROMOTER", inherits = TRUE, ifnotfound = FALSE)) &&
      "Promoter" %in% names(df)) {
    proms_present <- sort(unique(df$Promoter[!is.na(df$Promoter)]))
    if (length(proms_present) > 0L) {
      for (p in proms_present) {
        cohorts[[paste0("Donor1_", p)]]   <- df[!is.na(df$Donor) & df$Donor == "1" &
                                                !is.na(df$Promoter) & df$Promoter == p, , drop = FALSE]
        cohorts[[paste0("Donor4_", p)]]   <- df[!is.na(df$Donor) & df$Donor == "4" &
                                                !is.na(df$Promoter) & df$Promoter == p, , drop = FALSE]
        cohorts[[paste0("Combined_", p)]] <- df[!is.na(df$Promoter) & df$Promoter == p, , drop = FALSE]
      }
    } else {
      message("[v4.26] PRISM_BY_PROMOTER=TRUE but no Promoter values present; skipping promoter splits.")
    }
  }
  cond_order <- c("Control", "GoF", "LoF")
...
csv_dir <- "C:/Users/Administrator/Documents/PreCFU_Assembloids_20x/"   # [v4.25] PRE CSVs + _AQuA2.mat companions live here
# csv_dir <- "/Volumes/Arvin2TBsd/CalciumTesting/PreCFU_hCO/"           # [v4.20] local test (Mac)
# csv_dir <- "C:/Users/Administrator/Documents/PreCFU_hCO/"             # earlier hCO production (EC2/Windows)
mat_dir   <- "C:/Users/Administrator/Documents/Assembloids_POST/"       # [v4.25] POST AQuA2 outputs for Assembloids
# mat_dir <- "/Volumes/Arvin2TBsd/CalciumTesting/POST/"                 # [v4.20] local test (Mac)
# mat_dir <- "C:/Users/Administrator/Documents/POST/"                   # earlier hCO production (EC2/Windows)
companion_mat_dir <- "C:/Users/Administrator/Documents/PreCFU_Assembloids_20x/"   # [v4.25] fts source = _AQuA2.mat files in PreCFU (NOT POST)
# companion_mat_dir <- "/Volumes/Arvin2TBsd/CalciumTesting/PreCFU_hCO/" # [v4.20] local test (Mac)
# companion_mat_dir <- "C:/Users/Administrator/Documents/PreCFU_hCO/"   # earlier hCO production (EC2/Windows)
out_dir <- "C:/Users/Administrator/Documents/RESULTS_Assembloids/"      # [v4.25] dedicated results tree (kept separate from hCO RESULTS)
# out_dir <- "/Volumes/Arvin2TBsd/CalciumTesting/RESULTS/"              # [v4.20] local test (Mac)
# out_dir <- "C:/Users/Administrator/Documents/RESULTS/"                # earlier hCO production (EC2/Windows)
...
# actually in.)
TEST_MODE <- TRUE      # TRUE = small stratified subset first; FALSE = all recordings
TEST_N    <- 10L       # recordings to sample when TEST_MODE = TRUE
TEST_SEED <- 1L        # reproducible sample
GROUPING_VARS <- c("Condition", "DonorCond", "Timepoint", "CondTimepoint")   # [v4.16] PART P + modeling run once per entry
META_COLS <- c("Donor","Condition","DonorCond","AgeDay","Timepoint","CondTimepoint","Mag",
               "NominalHz","EffectiveHz","Tissue","Promoter","Organoid","Video")
...
# --- [hCO v1] position-independent stem parser (validated) ------------------
parse_stem <- function(stem) {
  toks <- strsplit(stem, "_", fixed = TRUE)[[1]]
  toks <- toks[nzchar(toks)]
  grab <- function(pat) {
    hit <- toks[grepl(pat, toks, perl = TRUE)]
    if (length(hit)) sub(pat, "\\1", hit[1], perl = TRUE) else NA_character_
  }
  donorcond <- grab("^([0-9]+[CGLcgl])$")                 # 4L, 1C, 1g
  if (!is.na(donorcond)) {
    donor <- sub("([0-9]+).*", "\\1", donorcond)
    cond  <- toupper(sub("[0-9]+", "", donorcond))
  } else {
    donor <- grab("^(?:[Dd]onor|D)([0-9]+)$")             # Donor4 / D4 (not d128)
    cond  <- { c <- grab("^([CGLcgl])$"); if (!is.na(c)) toupper(c) else NA_character_ }
  }
  age <- suppressWarnings(as.numeric(grab("^d([0-9]+)$")))            # d128 -> 128
  mag <- grab("^([0-9]+[xX])$")                                      # 20x
  org <- suppressWarnings(as.integer(grab("^[Oo][Rr][Gg]([0-9]+)$")))# ORG3
  vid <- suppressWarnings(as.integer(grab("^[Vv]([0-9]+)$")))        # V1
  tis <- grab("^(hCO|hco|HCO|ASSY|assy|ASMB|asmb|ASM|asm|[Aa]ssembloid|[Oo]rganoid)$")  # [hCO/assembloid] tissue tokens
  # [v4.26] Promoter token (CAMK2A / DLX / etc.).  Case-insensitive lookup
  # against PROMOTER_TOKENS (defined in config block).  Returns the matched
  # token uppercased for consistent grouping.
  prom_idx  <- if (exists("PROMOTER_TOKENS", inherits = TRUE))
                 which(toupper(toks) %in% toupper(PROMOTER_TOKENS)) else integer(0)
  prom <- if (length(prom_idx) >= 1L) toupper(toks[prom_idx[1]]) else NA_character_
  hz_tok <- toks[grepl("^[0-9.]+[Hh][Zz]$", toks)]
  hz_val <- suppressWarnings(as.numeric(sub("[Hh][Zz]$", "", hz_tok)))
  hz_val <- hz_val[is.finite(hz_val)]
  hz_int  <- hz_val[hz_val == round(hz_val)]
  hz_frac <- hz_val[hz_val != round(hz_val)]
  data.frame(
    Donor       = ifelse(is.na(donor), NA_character_, as.character(donor)),
    Condition   = cond,
    DonorCond   = if (!is.na(donor) && !is.na(cond)) paste0(donor, cond) else NA_character_,
    AgeDay      = age,
    Timepoint   = assign_timepoint(age),                                # [v4.16] D80 / D120 / NA
    CondTimepoint = { tp <- assign_timepoint(age);                      # [v4.16] e.g. "C_D80"
                      if (!is.na(cond) && !is.na(tp)) paste0(cond, "_", tp) else NA_character_ },
    Mag         = toupper(mag),
    NominalHz   = if (length(hz_int))  hz_int[1]  else NA_real_,
    EffectiveHz = if (length(hz_frac)) hz_frac[1] else NA_real_,
    Tissue      = ifelse(is.na(tis), NA_character_, tis),
    Promoter    = prom,                                                 # [v4.26] CAMK2A / DLX / ...
    Organoid    = org,
    Video       = vid,
...
n_unparsed <- sum(is.na(pairs$Donor) | is.na(pairs$Condition))
if (n_unparsed > 0)
  message("  [warn]  ", n_unparsed, " file(s) with unparsed Donor/Condition -- inspect: ",
          paste(head(pairs$key[is.na(pairs$Donor) | is.na(pairs$Condition)], 5), collapse = ", "))
